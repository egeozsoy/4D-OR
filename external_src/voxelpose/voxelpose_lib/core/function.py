...
from voxelpose_lib.utils.vis import save_debug_3d_cubes
from voxelpose_lib.utils.vis import save_debug_3d_images
from voxelpose_lib.utils.vis import save_debug_images_multi
...
def train_3d(config, model, optimizer, loader, epoch, output_dir, writer_dict, device=torch.device('cuda'), dtype=torch.float):
    ...
    for i, (inputs, targets_2d, weights_2d, targets_3d, meta, input_heatmap) in enumerate(loader):
        ...
        elif 'campus' in config.DATASET.TEST_DATASET or 'shelf' in config.DATASET.TEST_DATASET or 'OR_4D' in config.DATASET.TEST_DATASET:
        ...
        if i % config.PRINT_FREQ == 0:
            gpu_memory_usage = torch.cuda.memory_allocated(0) if torch.cuda.is_available() else 0
            msg = 'Epoch: [{0}][{1}/{2}]\t' \
