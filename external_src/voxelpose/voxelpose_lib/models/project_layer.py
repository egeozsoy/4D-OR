...
import voxelpose_lib.utils.cameras as cameras
from voxelpose_lib.utils.transforms import affine_transform_pts_cuda as do_transform
from voxelpose_lib.utils.transforms import get_affine_transform as get_transform
...
class ProjectLayer(nn.Module):
    def __init__(self, cfg):
        ...
        if 'OR_4D' in cfg.DATASET['TEST_DATASET']:
            self.project_pose_fn = cameras.project_pose_OR_4D
        else:
            self.project_pose_fn = cameras.project_pose
            ...
            if len(grid_center[0]) == 3 or grid_center[i][3] >= 0:
                ...
                for c in range(n):
                    ...
                    for k, v in meta[c]['camera'].items():
                        cam[k] = v[i]
                    xy = self.project_pose_fn(grid, cam)
