...
from .OR_4D_Dataset import OR_4DDataset

logger = logging.getLogger(__name__)


class OR_4DKeypoints(OR_4DDataset):
    def __init__(self,
                 cfg,
                 dataset_name,
        ...
        assert cfg.DATASET.NUM_JOINTS == 14, 'Number of joint for COCO is 17'
        ...
        if remove_images_without_annotations:
            self.ids = [image_dict['image_id'] for image_dict in self.image_dicts if len(image_dict['annotations']) > 0]

        self.transforms = transforms
        self.heatmap_generator = heatmap_generator
        self.joints_generator = joints_generator

    def show_keypoints(self, img, joints):
        plt.imshow(img)
        plt.scatter(joints[:, :, 0], joints[:, :, 1], color='r')
        plt.show()

    def show_keypoints_tensor(self, img, joints):
        img = img - img.min()
        img = img / img.max()
        img = img.numpy().transpose(1, 2, 0)
        self.show_keypoints(img, joints * 2)
