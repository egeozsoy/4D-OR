...
from higherhrnet_lib.dataset import VIS_CONFIG


def add_joints(image, joints, color, dataset='COCO', thickness=10):
    ...
    for joint in joints:
        if joint[2] > 0:
            cv2.circle(image, (int(joint[0]), int(joint[1])), 1, color, thickness)
...
def save_valid_image(image, joints, file_name, dataset='COCO'):
    ...
    for person in joints:
        color = np.random.randint(25, 200, size=3)
