...
class Batch():
    def __init__(self, idx, attn_bias, attn_edge_type, spatial_pos, in_degree, out_degree, x, edge_input, y, meta):
...
def collator(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20, dataset_name=''):
    meta = {}
    is_role_prediction = dataset_name == 'role_prediction'
    if is_role_prediction:
        assert len(items) == 1
        ys = [items[0][1]]
        idxs = items[0][4]
        meta['take_idx'] = items[0][2]
        meta['track_idx'] = items[0][3]
        items = items[0][0]
    items = [item for item in items if item is not None and item.x.size(0) <= max_node]
    if is_role_prediction:
        items = [(item.attn_bias, item.attn_edge_type, item.spatial_pos, item.in_degree, item.out_degree, item.x, item.edge_input[:, :, :multi_hop_max_dist, :],
                  item.is_target)
                 for item in items]
        attn_biases, attn_edge_types, spatial_poses, in_degrees, out_degrees, xs, edge_inputs, is_targets = zip(*items)
    else:
    ...
    if is_role_prediction:
        is_target = torch.cat([pad_2d_unsqueeze(i[:, None], max_node_num) for i in is_targets])
        meta['is_target'] = is_target
