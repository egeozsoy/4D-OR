...
from group_free_3D.utils.eval_det import eval_det_cls, eval_det_multiprocessing
from group_free_3D.utils.eval_det import get_iou_obb
from group_free_3D.utils.nms import nms_2d_faster, nms_3d_faster, nms_3d_faster_samecls
from group_free_3D.utils.box_util import get_3d_box
from group_free_3D.utils.sunrgbd_utils import extract_pc_in_box3d
from group_free_3D.OR_4D.OR_4D_detection_dataset import DC
from helpers.configurations import GROUP_FREE_PREDICTIONS_PATH

DUMP_CONF_THRESH = 0.25  # TODO maybe increase back to 0.5 # Dump boxes with obj prob larger than that.
...
def dump_predictions(end_points, config, prefix, scan_names):
    ''' Dump results.
    ...
    # NETWORK OUTPUTS
    objectness_scores = end_points[f'{prefix}objectness_scores'].detach().cpu().numpy()
    pred_center = end_points[f'{prefix}center'].detach().cpu().numpy()  # B,num_proposal,3
    pred_heading_class = torch.argmax(end_points[f'{prefix}heading_scores'], -1)  # B,num_proposal
    ...
    pred_sem_cls = torch.argmax(end_points[f'{prefix}sem_cls_scores'], -1).detach().cpu().numpy()  # B,num_proposal
    ...
    for i in range(batch_size):
        ...
        objectness_prob = sigmoid(objectness_scores[i, :, 0])  # (K,)
        predicted_class = pred_sem_cls[i]
        ...
        if np.sum(objectness_prob > DUMP_CONF_THRESH) > 0:
            ...
            if len(obbs) > 0:
                ...
                confident_mask = objectness_prob > DUMP_CONF_THRESH
                confident_nms_mask = np.logical_and(confident_mask, pred_mask[i, :] == 1)
                confident_bboxes_out = obbs[confident_mask]
                confident_bboxes_scores_out = objectness_prob[confident_mask]
                confident_bboxes_classes_out = predicted_class[confident_mask]
                confident_nms_bboxes_out = obbs[confident_nms_mask]
                confident_nms_bboxes_scores_out = objectness_prob[confident_nms_mask]
                confident_nms_bboxes_classes_out = predicted_class[confident_nms_mask]
                np.savez_compressed(str(save_name),
                                    {'bboxes': confident_bboxes_out, 'scores': confident_bboxes_scores_out, 'classes': confident_bboxes_classes_out,
                                     'bboxes_nms': confident_nms_bboxes_out, 'scores_nms': confident_nms_bboxes_scores_out,
                                     'classes_nms': confident_nms_bboxes_classes_out})
                # pred_nms_bboxes_out = obbs[pred_mask[i, :] == 1, :]
                # pred_bboxes_out = obbs
                # pc_util.custom_write_oriented_bbox(*confident_bboxes_out)


def dump_ground_truth(end_points, config, scan_names):
    ''' Dump results.
    ...
    center_label = end_points['center_label'].detach().cpu().numpy()  # B,num_proposal,3
    # pred_heading_class = torch.argmax(end_points[f'{prefix}heading_scores'], -1)  # B,num_proposal
    ...
    for i in range(batch_size):
        scan_name = scan_names[i]
        save_name = GROUP_FREE_PREDICTIONS_PATH / f'{scan_name}_GT.npz'
        gt_class = sem_cls_label[i]
        mask = box_label_mask[i].astype(np.bool)
        # Dump predicted bounding boxes
        obbs = []
        for j in range(len(gt_class)):
            if mask[j] == 0:
                continue

            obb = config.param2obb(center_label[i, j, 0:3], heading_class_label[i, j], heading_residual_label[i, j],
                                   size_class_label[i, j], size_residual_label[i, j])
            obbs.append(obb)
        if len(obbs) > 0:
            obbs = np.vstack(tuple(obbs))  # (num_proposal, 7)
            bboxes_out = obbs
            bboxes_classes_out = gt_class[mask]
            np.savez_compressed(str(save_name),
                                {'bboxes': bboxes_out, 'classes': bboxes_classes_out})
...
class APCalculator(object):
    ...
    def compute_metrics(self):
        ...
        elems = [elem for elem in ap.values() if elem > 0.0]
        ret_dict['mAP_nonzero'] = np.mean(elems) if len(elems) else 0.0
        rec_list = []
        for key in sorted(ap.keys()):
            clsname = self.class2type_map[key] if self.class2type_map else str(key)
            rec_key = key
            try:
                ret_dict['%s Recall' % (clsname)] = rec[rec_key][-1]
                rec_list.append(rec[rec_key][-1])
            except:
                ret_dict['%s Recall' % (clsname)] = 0
                rec_list.append(0)
        ret_dict['AR'] = np.mean(rec_list)
        elems = [elem for elem in rec_list if elem > 0.0]
        ret_dict['AR_nonzero'] = np.mean(elems) if len(elems) else 0.0
        return ret_dict
